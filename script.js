...
function toggleVoiceMode() {
    // VERBESSERT: Guard clause to prevent errors if speech API failed to initialize
    if (!speechRecognition) {
        showNotification('Spracherkennung wird von diesem Browser nicht unterstützt oder wurde blockiert.', 'error');
        return;
    }
    ...
    if (isVoiceModeActive) {
        ...
        // Create a snapshot of the history for the voice session
...
function startListening() {
    // VERBESSERT: More robust check before starting
    if (!isVoiceModeActive || isListening || !speechRecognition) return;

    // Ensure any previous speech is stopped before listening
    ...
    try {
        ...
        console.error("Fehler beim Starten der Spracherkennung (möglicherweise bereits aktiv):", e);
...
async function handleVoiceResult(event) {
    ...
    isListening = false; // Recognition has produced a result

    if (!transcript) {
        if (isVoiceModeActive) startListening(); // If result is empty, just listen again
        return;
    }
    ...
    voiceChatBtn.classList.remove('listening');
    voiceChatBtn.innerHTML = '<div class="loading"></div>'; // Use loading spinner
...
async function handleVoiceResult(event) {
    // VERBESSERT: Dies ist die wichtigste Änderung.
    // Wenn der Bot gerade spricht, wird jedes Ergebnis der Spracherkennung ignoriert.
    if (isBotSpeaking) {
...
function speakText(textToSpeak) {
    ...
    // VERBESSERT: Wir aktivieren den "Master-Schalter" direkt vor der Sprachausgabe.
    isBotSpeaking = true; 
    ...
    // Sucht die vom Benutzer ausgewählte Stimme
    let selectedVoice = germanVoices.find(voice => voice.name === currentVoiceName);

    // Fallback: Wenn die gespeicherte Stimme nicht gefunden wird,
    ...
    // VERBESSERT: Der 'onend' Event ist jetzt dafür verantwortlich, den "Master-Schalter" wieder umzulegen.
    utterance.onend = () => {
        isBotSpeaking = false; // Sprachausgabe beendet, Echo-Gefahr vorüber.
        if (isVoiceModeActive) {
            startListening(); // JETZT ist es sicher, wieder zuzuhören.
        }
    };
    
    utterance.onerror = (event) => {
        console.error('Fehler bei der Sprachausgabe:', event.error);
        isBotSpeaking = false; // Auch bei einem Fehler den Status zurücksetzen.
        if (isVoiceModeActive) {
            startListening(); // Trotzdem versuchen, weiterzumachen.
        }
    };
