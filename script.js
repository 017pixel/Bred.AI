...
async function handleVoiceResult(event) {
    ...
    // --- ÄNDERUNG HIER ---
    // Wir zeigen nicht mehr "Du sagtest:", sondern nur noch einen dezenten Hinweis.
    voiceStatus.textContent = "Verarbeite...";
    voiceTranscript.textContent = ''; // Das Transkriptfeld wird geleert.
    // --- ENDE DER ÄNDERUNG ---
...
function speakText(textToSpeak) {
    ...
    voiceInterruptBtn.style.display = 'flex'; // Button zuverlässig einblenden
    ...
    const cleanupAndRestart = () => {
        isBotSpeaking = false;
        voiceInterruptBtn.style.display = 'none'; // Button ausblenden
        if (isVoiceModeActive) {
            startListening(); 
        }
    };
