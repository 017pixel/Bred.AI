...
function speakText(textToSpeak) {
    // VERBESSERT: Guard clause to prevent errors
    ...
    const utterance = new SpeechSynthesisUtterance(cleanedText); // Speak the clean text
    utterance.lang = 'de-DE';
    utterance.rate = voiceRate;
    utterance.pitch = voicePitch;

    // Use the user-selected or preferred default voice
    let selectedVoice = germanVoices.find(voice => voice.name === currentVoiceName);
    if (selectedVoice) {
        utterance.voice = selectedVoice;
    } else if (germanVoices.length > 0) {
        utterance.voice = germanVoices[0]; // Fallback to the first available German voice
        console.warn(`Gespeicherte Stimme "${currentVoiceName}" nicht gefunden. Nutze Fallback.`);
    }

    // VERBESSERT: This is the single point of truth for restarting the listening loop.
    utterance.onend = () => {
        if (isVoiceModeActive) {
            startListening();
        }
    ...
    utterance.onerror = (event) => {
        ...
        // Even if speaking fails, restart listening so the user isn't stuck
        if (isVoiceModeActive) {
            startListening();
        }
    };
