...
async function handleVoiceResult(event) {
    ...
    // Zusätzliche Sicherheit: Stoppe die Erkennung explizit.
...
function speakText(textToSpeak) {
    ...
    // --- ENTSCHEIDENDE ÄNDERUNG HIER ---
    // Wir rufen startListening() nicht mehr direkt auf, sondern mit dem Sicherheitspuffer.
    utterance.onend = () => {
        isBotSpeaking = false; 
        if (isVoiceModeActive) {
            setTimeout(() => {
                startListening();
            }, MOBILE_SAFETY_DELAY); // Der Sicherheitspuffer wird hier verwendet!
        }
    };
    
    // Wir bauen den Puffer auch für den Fehlerfall ein, nur zur Sicherheit.
    utterance.onerror = (event) => {
        console.error('Fehler bei der Sprachausgabe:', event.error);
        isBotSpeaking = false; 
        if (isVoiceModeActive) {
