...
async function saveProfile() {
    ...
    try {
        ...
        alert("Das Profil konnte nicht gespeichert werden. Bitte versuche es erneut.");
    } finally {
...
function initializeSpeechAPI() {
    ...
    // Prüfen, ob die Browser-APIs für Spracheingabe und -ausgabe verfügbar sind
    if (!SpeechRecognition || !speechSynth || !window.SpeechSynthesisUtterance) {
        console.warn("Web Speech API wird von diesem Browser nicht vollständig unterstützt. Der Sprachmodus wird deaktiviert.");
        
        const voiceChatButton = document.getElementById('voice-chat-btn');
        if (voiceChatButton) {
            // Button ausblenden, da das Feature nicht nutzbar ist
            voiceChatButton.style.display = 'none';
        }
        
        // Den Benutzer nur einmal pro Sitzung informieren, um Pop-up-Spam zu vermeiden
        if (!sessionStorage.getItem('voiceSupportWarningShown')) {
            alert('Ihr Browser unterstützt den Sprachmodus nicht oder nicht vollständig. Das Feature wird daher deaktiviert.');
            sessionStorage.setItem('voiceSupportWarningShown', 'true');
        }
        return; // Initialisierung hier abbrechen
    }
    
    // Wenn alles unterstützt wird, fahre mit der Initialisierung fort
...
function getPersonalizedSystemPrompt(isForVoice = false) {
    ...
    // Dynamisches Füllen der Custom-Bot-Liste
    if (currentBot === 'bred') {
