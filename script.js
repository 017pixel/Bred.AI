...
const BOT_PERSONALITIES = {
    "bred": { "name": "Bred", "emoji": "😊", "prompt": "Du bist Bred 😊 - freundlich, hilfsbereit, präzise. Unterhalte {name}, löse Probleme. Schreibstil: motivierend, klar, kurze Antworten (2-3 Sätze), bei Erklärungen länger (5-10 Sätze). Nutze Emojis in Überschriften, gib konkrete Tipps, fasse wichtige Punkte zusammen. Begrüße nur in der ersten Antwort." },
    ...
    "weather": { "name": "WeatherBot", "emoji": "🌦️", "prompt": "Du bist der WeatherBot. Deine Aufgabe ist es, Wetterinformationen bereitzustellen. Analysiere die Benutzereingabe, um einen Ort zu identifizieren. Wenn kein Ort genannt wird, frage danach. Gib die Wetterdaten, die du erhältst, in einem freundlichen, gesprächigen Ton wieder." }
};
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        alert("Die Anwendung konnte nicht initialisiert werden. Bitte versuchen Sie, die Seite neu zu laden. Fehler: " + error.message);
    }
});
...
function loadAndCacheVoices() {
    return new Promise((resolve) => {
        ...
        const findAndSetVoices = () => {
            ...
            // 1. Sofort-Versuch: Wenn Stimmen da sind, verarbeite sie.
            if (allVoices.length > 0) {
                germanVoices = allVoices.filter(voice => voice.lang === 'de-DE');
                voiceSelect.innerHTML = ''; // Leere die Liste für den Fall, dass sie neu befüllt wird

                if (germanVoices.length > 0) {
                    console.log(`[Versuch ${attempts+1}] Stimmen gefunden. Verarbeite ${germanVoices.length} deutsche Stimmen.`);
                    
                    // Fülle die Dropdown-Liste
                    ...
                    // *** DER ENTSCHEIDENDE TEIL: PRIORISIERE GOOGLE ***
                    let preferredVoice = germanVoices.find(v => v.name.toLowerCase().includes('google'));
                    
                    // Wenn es eine gespeicherte Stimme gibt und sie existiert, nutze sie.
                    if (currentVoiceName && germanVoices.some(v => v.name === currentVoiceName)) {
                        voiceSelect.value = currentVoiceName;
                    } 
                    // Sonst, wenn eine Google-Stimme gefunden wurde, setze sie als Standard.
                    else if (preferredVoice) {
                        currentVoiceName = preferredVoice.name;
                        voiceSelect.value = currentVoiceName;
                        console.log("Bevorzugte Google-Stimme gefunden und gesetzt:", currentVoiceName);
                    } 
                    // Sonst nimm einfach die erste verfügbare deutsche Stimme.
                    else {
                        currentVoiceName = germanVoices[0].name;
                        voiceSelect.value = currentVoiceName;
                    }

                    resolve(); // Wir sind fertig!
                    return true; // Signalisiert, dass die Suche erfolgreich war.
                }
            }
            
            // 2. Fallback-Mechanismus: Wenn keine Stimmen gefunden wurden.
            attempts++;
            console.log(`[Versuch ${attempts}] Keine Stimmen geladen. Versuche es in 250ms erneut.`);
            if (attempts < 10) { // Versuche es maximal 10 Mal (ca. 2.5 Sekunden)
                setTimeout(findAndSetVoices, 250);
            } else {
                console.error("Kritischer Fehler: Konnte nach mehreren Versuchen keine Stimmen laden.");
                voiceSelect.innerHTML = '<option>Stimmen nicht verfügbar</option>';
                resolve(); // Beende den Prozess, damit die App nicht abstürzt.
            }
            return false;
        };

        // Starte den ersten Versuch sofort.
        findAndSetVoices();

        // Ein zusätzliches Sicherheitsnetz: Das offizielle 'onvoiceschanged'-Event.
        // Falls es doch noch feuert, stellen wir sicher, dass die Liste korrekt ist.
        speechSynthesis.onvoiceschanged = () => {
            console.log("'onvoiceschanged' Event gefeuert. Überprüfe die Stimmenliste erneut.");
            findAndSetVoices();
        };
    });
}
...
function checkFirstVisit() {
    if (Object.keys(profiles).length === 1 && profiles[currentProfileId].name === 'Temporäres Profil') {
        setTimeout(() => openCreateProfileModal(), 1000);
    }
}
...
async function deleteProfile(profileIdToDelete) {
    if (Object.keys(profiles).length <= 1) {
        alert("Du kannst das letzte verbleibende Profil nicht löschen.");
        return;
    }
...
function selectBot(botId) {
    if (activeProjectId) {
        alert("Bot kann nicht geändert werden, während ein Projekt aktiv ist.");
        return;
    }
    currentBot = botId;
...
function selectModel(modelId) {
    if (activeProjectId) {
        alert("Modell kann nicht geändert werden, während ein Projekt aktiv ist.");
        return;
    }
...
async function saveCustomBot() {
    ...
    if (!name || !displayText || !prompt) { alert("Bitte fülle alle Felder aus."); return; }
...
function handleProjectFileSelect(event) {
    ...
    if (projectModalFiles.length + newFiles.length > MAX_PROJECT_FILES) {
        alert(`Fehler: Ein Projekt kann maximal ${MAX_PROJECT_FILES} Dateien enthalten.`);
...
async function saveProject() {
    ...
    if (!name) { alert("Bitte gib einen Namen für das Projekt ein."); return; }
...
async function getWeatherForCity(cityName) {
    try {
        showSearchStatus(`🌍 Suche nach Koordinaten für ${cityName}...`);
        const geoResponse = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(cityName)}&count=1`);
        if (!geoResponse.ok) throw new Error('Stadt nicht gefunden.');
        
        const geoData = await geoResponse.json();
        if (!geoData.results || geoData.results.length === 0) return `Ich konnte leider keine Stadt namens "${cityName}" finden.`;
        
        const { latitude, longitude, name, country } = geoData.results[0];
        showSearchStatus(`🌦️ Frage Wetter für ${name}, ${country} ab...`);
        ...
        return `Das aktuelle Wetter in **${name}, ${country}**: Es sind **${Math.round(temperature)}°C** bei ${weatherDescription}.`;

    } catch (error) {
        console.error("Wetter-Fehler:", error);
        hideSearchStatus();
        return `Entschuldigung, es gab ein Problem beim Abrufen der Wetterdaten für "${cityName}".`;
    }
}

function getWeatherDescription(code) {
    const descriptions = { 0: "klarem Himmel ☀️", 1: "überwiegend klarem Himmel 🌤️", 2: "teilweise bewölktem Himmel ⛅", 3: "bedecktem Himmel ☁️", 45: "Nebel 🌫️", 48: "Raureif-Nebel 🌫️", 51: "leichtem Nieselregen 💧", 53: "mäßigem Nieselregen 💧", 55: "starkem Nieselregen 💧", 61: "leichtem Regen 🌦️", 63: "mäßigem Regen 🌧️", 65: "starkem Regen 🌧️", 80: "leichten Regenschauern 🌦️", 81: "mäßigen Regenschauern 🌧️", 82: "starken Regenschauern ⛈️", 95: "einem Gewitter ⛈️" };
    return descriptions[code] || "einem nicht definierten Wetterphänomen";
}
...
async function sendMessage() {
    ...
    try {
        ...
        } else if (currentBot === 'weather') {
            botResponse = await getWeatherForCity(message);
        } else {
            const modelInfo = MODELS[currentModel];
...
function getPersonalizedSystemPrompt(isForVoice = false) {
    ...
    if (userInterests.length > 0) systemPrompt += `\nInteressen: ${userInterests.join(', ')}.`;

    if (isForVoice) {
        systemPrompt += "\n\nWICHTIGE ANWEISUNG: Deine Antwort wird von einer Text-to-Speech-Stimme vorgelesen. Formuliere deine Antwort daher natürlich und gesprächig. VERWENDE ABSOLUT UNTER KEINEN UMSTÄNDEN NIE EMOJIES, Sternchen, oder andere Markdown-Formatierungen. Antworte nur mit reinem, fließendem Text.";
    }
...
function showHelp() { alert('BredAI Hilfe:\n\n- Projekte: Erstelle Wissensdatenbanken für spezifische Themen.\n- Wähle Bots & KI-Modelle.\n- Erstelle eigene Bots & Profile.\n- Personalisiere mit Interessen.\n- Lade alte Chats aus dem Verlauf.\n- Dark Mode & mehr in Einstellungen.'); }
function showAbout() { alert('🤖 BredAI 2025\nDein anpassbarer KI-Chat-Assistent. Mit Pro-Features, kostenlos! Version 21.1'); }

// --- SPRECHERKENNUNG & STIMME ---

function initializeSpeechAPI() {
    // Schritt 1: Prüfe, ob BEIDE Teile der API (Input & Output) vorhanden sind.
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    const speechSynth = window.speechSynthesis;
    const SpeechSynthesisUtterance = window.SpeechSynthesisUtterance;

    // Wenn einer der Teile fehlt, wird der Sprachmodus komplett deaktiviert.
    if (!SpeechRecognition || !speechSynth || !SpeechSynthesisUtterance) {
        // Verstecke den Button, damit der Nutzer ihn gar nicht erst klicken kann.
        if (voiceChatBtn) {
            voiceChatBtn.style.display = 'none';
        }
        // Gib eine klare Meldung in der Entwicklerkonsole aus.
        console.warn("Web Speech API (Erkennung oder Synthese) wird von diesem Browser/dieser Ansicht nicht unterstützt. Sprachmodus wird deaktiviert.");
        // Beende die Funktion hier, um Fehler zu vermeiden.
        return;
    }
    
    // Wenn alles vorhanden ist, fahre mit der Initialisierung fort.
    ...
    speechRecognition.onend = () => {
        isListening = false;
        // Wichtiger Hinweis: Dieser automatische Neustart funktioniert auf iOS nicht wie erwartet.
...
async function handleVoiceResult(event) {
    ...
    voiceChatBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
...
// ERSETZE DEINE ALTE 'handleVoiceError' FUNKTION:
function handleVoiceError(event) {
    ...
    // Dieser Fehler tritt auf, wenn die Mikrofon-Berechtigung fehlt.
    ...
    // Für andere Fehler eine kurze Meldung anzeigen und dann erneut versuchen.
...
function speakText(textToSpeak) {
    ...
    // NEU: Sucht die vom Benutzer ausgewählte Stimme
    let selectedVoice = germanVoices.find(voice => voice.name === currentVoiceName);

    // Fallback: Wenn die gespeicherte Stimme nicht gefunden wird (z.B. anderer Browser),
    // wird die erste verfügbare deutsche Stimme genommen.
    if (selectedVoice) {
